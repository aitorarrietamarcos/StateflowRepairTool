function baselineAlgorithm(seed,faultyModel, nonFaultyModel, executeTest, current_Path)
...
%%
    ...
    while toc<timeBudget %&& ~plausiblePatchFound
        ...
        while  toc<timeBudget
            ...
            copyfile(strcat(faultyModel, '.slx'), strcat(faultyModel, '_', num2str(numOfIterations), '.slx')); 
            open_system(strcat(faultyModel, '_', num2str(numOfIterations), '.slx'));
            done = false;
            while done == false
            ...
            try
                save_system(strcat(faultyModel, '_', num2str(numOfIterations), '.slx'));
            catch
                
            end
            bdclose(strcat(faultyModel, '_', num2str(numOfIterations), '.slx'));
            %bdclose(modelname)
            try
                [verdict,timeVerdictActive,criticalityVerdict,timeFirstFailureExhibited] = executeTest( strcat(faultyModel, '_', num2str(numOfIterations) ));
                if sum(verdict)==0
                    plausiblePatchFound =true;
                    numsOfPlausiblePatches = numsOfPlausiblePatches+1;
                    PlausiblePatches(numsOfPlausiblePatches) = strcat(faultyModel, '_', num2str(numOfIterations));
                    disp(strcat('Plausible patch found! This one it is = ' ,faultyModel, '_', num2str(numOfIterations)));
                elseif (timeVerdictActive<Archive{selectedModelToMutate}.timeVerdict || criticalityVerdict<Archive{selectedModelToMutate}.criticality || timeFirstFailureExhibited>Archive{selectedModelToMutate}.firstFailureExhibited )...
                    ...
                    faultyModel = strcat(faultyModel, '_', num2str(numOfIterations));
                    plausiblePatchFound = false;
            ...
            bdclose(nonFaultyModel);
            bdclose(strcat(faultyModel, '_', num2str(numOfIterations)));
            bdclose('all')
        end
